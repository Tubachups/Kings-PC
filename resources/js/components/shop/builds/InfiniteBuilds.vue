    ...
    <InfiniteScroll data="builds" :buffer="300" manual preserve-url>
        ...
        <template #next="{ loading, fetch, hasMore }">
            <button
                v-if="hasMore"
                :disabled="loading"
                class="relative mx-auto mt-6 flex items-center justify-center rounded-lg border border-border bg-primary px-6 py-2 font-semibold text-primary-foreground shadow-lg transition-all duration-200 hover:bg-primary/90 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-primary/50 cursor-pointer"
                @click="fetch"
            >
