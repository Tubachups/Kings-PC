...
const stepIndex = ref(checkoutStore.stepIndex);
...
const currentSchema = computed(() => {
    ...
    if (stepIndex.value === 2)
        return toTypedSchema(step1Schema.merge(step2Schema));
...
watch(
    stepIndex,
    (value) => {
        checkoutStore.setStepIndex(value);
    },
    { immediate: true },
);
    ...
    >
        <Stepper v-model="stepIndex" class="block w-full">
            <form
                ...
                "
            >
                <div class="flex-start mb-8 flex w-full gap-2">
                    <StepperItem
                    ...
                    >
                        ...
                        <StepperTrigger as-child>
                            <Button
                                ...
                                tabindex="-1"
                                aria-disabled="true"
                                class="pointer-events-none z-10 rounded-full"
                            >
                                <Check
                        ...
                        <div class="mt-2 text-center">
                            <StepperTitle class="text-sm font-bold">{{
                                step.title
                            }}</StepperTitle>
                        </div>
                    </StepperItem>
                ...
                <div class="mt-10 flex justify-between">
                    <Button
                        ...
                        @click="stepIndex--" class="cursor-pointer"
                        >Back</Button>
                    <Button
                        v-if="stepIndex < 3"
                        type="button"
                        @click="
                            async () => {
                                ...
                                if (res.valid) stepIndex++;
                            }
                        " class="cursor-pointer"
                        >Next</Button>
                    <Button v-else :disabled="isSubmitting" type="submit" class="cursor-pointer">
