...
import { CheckCircle2, Mail, Package } from 'lucide-vue-next';
import { computed, ref } from 'vue';
import { index as productsIndex, deliveredOrders as deliveredOrdersAction } from '@/actions/App/Http/Controllers/Admin/ProductController';
...
const changePage = (page: number) => {
    router.get(deliveredOrdersAction().url, { page, per_page: props.orders.per_page }, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    });
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Delivered Orders</h1>
                    <p class="text-muted-foreground">
                        Review completed and delivered customer orders.
                    </p>
                </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <Card v-for="order in formattedOrders" :key="order.id">
                    ...
                    <CardContent class="space-y-2 text-sm">
                        ...
                        <p class="text-xs text-muted-foreground">Delivered: {{ formatDate(order.created_at || '') || '-' }}</p>
                    </CardContent>
                </Card>
            </div>
